...
const Extractor = () => {
  ...
  const handleCanvasMouseMove = useCallback((event) => {
    if (manualDrawState.isDrawing && manualDrawState.start) {
      ...
      setManualDrawState(prev => ({ ...prev, rect: { left, top, width, height } }));
    }
  ...
  const redrawCanvas = useCallback(() => {
      ...
      if (!manualDrawState.isDrawing) {
        ...
        Object.entries(dataPrepState.fieldMappings).forEach(([field, coords]) => {
          if (coords && coords.manual) {
            ...
            ctx.lineWidth = 2;
            ctx.setLineDash([2, 2]);
            ctx.strokeRect(coords.left * dataPrepState.currentZoom, coords.top * dataPrepState.currentZoom, coords.width * dataPrepState.currentZoom, coords.height * dataPrepState.currentZoom);
            ctx.restore();
          }
        });
      }
    ...
    if (manualDrawState.isDrawing && manualDrawState.rect) {
      ...
      ctx.strokeRect(left * dataPrepState.currentZoom, top * dataPrepState.currentZoom, width * dataPrepState.currentZoom, height * dataPrepState.currentZoom);
      ctx.restore();
    }
